      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                  ...
                  <p key={idx}>
                    {para.split(/(\*\*.*?\*\*)/g).map((part, i) =>
                      part.startsWith("**") && part.endsWith("**") ? (
                        <strong key={i} className="text-blue-400 font-bold">{part.slice(2, -2)}</strong>
                      ) : part
                    )}
                  </p>
                ))}
            ...
            <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-3">
              {portfolioData.profile.aboutBio.bulletPoints.map((point, idx) => (
                <div key={idx} className="flex items-center gap-3 p-3 bg-white/5 rounded-xl border border-white/5 group/point hover:bg-white/10 transition-colors">
                  <div className="w-1.5 h-1.5 rounded-full bg-blue-500 group-hover/point:scale-150 transition-transform" />
          ...
          >
            ...
            <div className="relative h-1/2 overflow-hidden bg-[#030014] group/loc">
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,#3b82f615_0%,transparent_70%)]" />
              ...
              <div className="absolute bottom-0 left-0 p-6 z-10 w-full bg-gradient-to-t from-[#030014] via-[#030014]/60 to-transparent">
            ...
            <div className="relative h-1/2 overflow-hidden flex flex-col justify-center bg-[#050510] group/tech border-t border-white/5">
              {/* Futuristic HUD Brackets */}
              <div className="absolute top-2 left-2 w-4 h-4 border-t-2 border-l-2 border-blue-500/30 group-hover/tech:border-blue-400 transition-colors z-20" />
              <div className="absolute top-2 right-2 w-4 h-4 border-t-2 border-r-2 border-blue-500/30 group-hover/tech:border-blue-400 transition-colors z-20" />
              <div className="absolute bottom-2 left-2 w-4 h-4 border-b-2 border-l-2 border-blue-500/30 group-hover/tech:border-blue-400 transition-colors z-20" />
              <div className="absolute bottom-2 right-2 w-4 h-4 border-b-2 border-r-2 border-blue-500/30 group-hover/tech:border-blue-400 transition-colors z-20" />
              ...
              <div className="z-20 absolute top-4 left-6">
                <div className="flex items-center gap-2 mb-1">
                  <div className="relative">
                    <Cpu className="w-3 h-3 text-blue-400 group-hover/tech:rotate-90 transition-transform duration-500" />
                    <div className="absolute inset-0 bg-blue-500/40 blur-sm animate-pulse rounded-full" />
                  </div>
                  <h4 className="text-[10px] font-black text-white tracking-widest uppercase italic flex items-center gap-2">
                    TECH_STACK
                    <span className="inline-block w-1 h-1 rounded-full bg-blue-500 animate-ping" />
                  </h4>
                </div>
              ...
              <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col gap-1 opacity-20 group-hover/tech:opacity-40 transition-opacity z-20">
                {[...Array(6)].map((_, i) => (
                  <div key={i} className={`h-1 w-1 rounded-full ${i % 2 === 0 ? 'bg-blue-400' : 'bg-gray-600'}`} />
                ))}
              </div>
              ...
              {/* Background Glow */}
              <div className="absolute inset-0 bg-gradient-to-br from-blue-600/10 via-transparent to-purple-600/10 opacity-0 group-hover/tech:opacity-100 transition-opacity duration-700 pointer-events-none" />
              <div className="absolute bottom-0 left-0 w-full h-[2px] bg-gradient-to-r from-transparent via-blue-500/50 to-transparent scale-x-0 group-hover/tech:scale-x-100 transition-transform duration-700" />
            </div>
          </motion.div>
          ...
          <div className="flex flex-wrap gap-3 relative z-10">
            {portfolioData.profile.techArsenal.map((tech) => (
              <motion.div
                ...
                className="px-6 py-3 bg-white/5 border border-white/10 rounded-xl text-xs font-bold text-gray-300 hover:text-white hover:border-blue-500/30 transition-all cursor-default flex items-center gap-3 group/skill shadow-sm"
              >
                <div className="w-1.5 h-1.5 rounded-full bg-blue-500/50 group-hover/skill:bg-blue-400 transition-colors" />
                {tech}
              </motion.div>
            ))}
          </div>
          ...
          {portfolioData.profile.values.map((value, index) => {
            ...
            return (
              <motion.div
                key={index}
                variants={systemInitVariants}
                className="group relative bg-white/5 backdrop-blur-3xl border border-white/10 rounded-2xl p-8 hover:bg-blue-500/5 hover:border-blue-500/20 transition-all duration-500 overflow-hidden"
              >
                {/* Sector Corners */}
                <div className="absolute top-2 left-2 w-4 h-4 border-t border-l border-white/20 group-hover:border-blue-500/50 transition-colors" />
                <div className="absolute bottom-2 right-2 w-4 h-4 border-b border-r border-white/20 group-hover:border-blue-500/50 transition-colors" />

                <div className="w-14 h-14 bg-gradient-to-br from-blue-500/10 to-indigo-500/10 border border-white/10 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 group-hover:border-blue-400/30 transition-all relative overflow-hidden">
                  <Icon className="w-6 h-6 text-blue-400 group-hover:rotate-12 transition-transform" />
                ...
                {/* Micro-Scanner Sweep */}
                <motion.div
                  className="absolute bottom-0 left-0 w-full h-[1px] bg-blue-500/40"
                  initial={{ scaleX: 0 }}
                  whileHover={{ scaleX: 1 }}
                />
              </motion.div>
