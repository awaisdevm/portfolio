...
export function createTranslator(dictionary: Dictionary, fallback?: Dictionary): TranslateFn {
    return function translate<T = string>(key: string, options?: TranslateOptions): T {
        ...
        // Safe check if dictionary exists
        let value = dictionary ? getNestedValue(dictionary, key) : undefined;
        ...
        if (value === undefined || value === null) {
            // Return empty string or fallback key instead of crashing DOM
            return (mode === 'empty' ? "" : key) as unknown as T;
        }

        if (options?.returnObjects) return value as T;
        if (typeof value !== 'string') return String(value) as unknown as T;

        return formatString(value, options?.params) as unknown as T;
    };
}